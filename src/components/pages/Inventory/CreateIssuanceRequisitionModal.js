...
const validationSchema = Yup.object().shape({
  // issuanceDescp: Yup.string().required("Categroy Name is required"),
...
const CreateIssuanceRequisitionModal = ({
  ...
  const onSubmit = async (values, { resetForm }) => {
    ...
    if (isEdit) {
      ...
      dattaaa.issuenceAutoId = rowData.issuenceAutoId;
      dattaaa.issuenceId = rowData.issuenceId;
      // dattaaa.issuenceDescp = rowData.issuenceDescp;
      dattaaa.validityCheck = rowData.issuenceAutoId;
    }
    console.log("from front", rowData);
    const requestData = {
      issuanceDescp: values.issuanceDescp,
      status: values.status,
      // validityCheck: values.validityCheck,
      validityCheck: 0,
      equipList: extractedProperties,
    };
    console.log("for POST req form issuance", requestData);
    const requestDataa = {
      // issuanceDescp: values.issuanceDescp,
      status: values.status,
      equipList: extractedProperties,
    ...
    console.log("for put req form issuance", requestDataa);
    const token = localStorage.getItem("token");
    try {
      if (isEdit) {
        ...
        const response = await fetch(
          `http://localhost:8007/api/Issuences/${rowData.issuenceAutoId}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
      ...
      if (!isEdit) {
        ...
        const data = await response.status;
        console.log("responce from post ISSUANCE", data);
      }
    } catch (err) {
  ...
  return (
    ...
    >
      <Box sx={style}>
        <Typography id="transition-modal-title" variant="h4" component="h4">
          {isEdit ? "Edit Issuance Requisition" : "Create Issuance Requisition"}
        </Typography>
        <Formik
            ...
            <Form onSubmit={handleSubmit}>
              {!isEdit && (
                <>
                  <TextField
                    name="issuanceDescp"
                    label="Issuance Description"
                    variant="outlined"
                    fullWidth={false}
                    margin="dense"
                    value={values.issuanceDescp}
                    onChange={handleChange}
                    onBlur={handleBlur}
                    error={
                      touched.issuanceDescp && Boolean(errors.issuanceDescp)
                    }
                    helperText={touched.issuanceDescp && errors.issuanceDescp}
                    color="warning"
                    size="small"
                    style={{ width: "22ch" }}
                    InputProps={{ style: { height: "40px", fontSize: "15px" } }}
