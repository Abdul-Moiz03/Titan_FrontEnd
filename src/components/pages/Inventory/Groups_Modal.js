...
const Groups_Modal = ({ isEdit, rowData, isOpen, onClose }) => {
  const onSubmit = async (values, { resetForm }) => {
    ...
    if (isEdit) {
      ...
      dattaaa.groupID = rowData.groupID;
    }
    ...
    try {
      if (isEdit) {
        ...
        const response = await fetch(
          `http://localhost:8007/api/Groups/${rowData.groupAutoID}`,
          {
            method: "PUT",
            headers: {
