...
const validationSchema = Yup.object().shape({
  pName: Yup.string().required("Group Name is required"),
  tom: Yup.string().required("Type of Maintenance is required"),
  assetName: Yup.string().required("Asset Name is required"),
});

const AddProcedureModal = ({ isEdit, rowData, isOpen, onClose }) => {
  const onSubmit = async (values, { resetForm }) => {
    const statusValue = values.status ? "Active" : "UnActive";
    const dattaaa = {};
    if (isEdit) {
      dattaaa.companyId = rowData.companyId;
      dattaaa.pAutoId = rowData.pAutoId;
    }
    const requestData = {
      pName: values.pName,
      tom: values.tom,
    ...
    try {
      if (isEdit) {
        ...
        const response = await fetch(
          `http://localhost:8007/api/Procedures?id=${rowData.pAutoId}`,
          {
            method: "PUT",
            headers: {
          ...
          {({
            ...
            <Form onSubmit={handleSubmit}>
              ...
              <TextField
                ...
                onChange={handleChange}
                onBlur={handleBlur}
                error={touched.tom && Boolean(errors.tom)}
                helperText={touched.tom && errors.tom}
                color="warning"
                size="small"
                style={{ width: "22ch" }}
                InputProps={{ style: { height: "40px", fontSize: "15px" } }}
